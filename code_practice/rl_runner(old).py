...
import json
import cv2
import argparse                     # <-- new import for CLI parsing
import numpy as np
...
class Util:
    ...
    def is_writable(path: Path) -> bool:
        """Return True if *path* can be created/written to."""
        try:
    ...
    def ask_choice(title: str, options: List[str]) -> str:
        """Prompt the user to choose one option from *options*."""
        print(f"\n== Choose {title} ==")
    ...
    def ask_int(title: str, default: int) -> int:
        """Prompt for an integer, falling back to *default*."""
    ...
    def linear_schedule(initial_value: float, final_value: float) -> Callable[[float], float]:
        """Return a callable that linearly interpolates between two values."""
    ...
    def to_uint8(arr: np.ndarray) -> np.ndarray:
        """Convert any numeric array to ``uint8`` (0‑255)."""
...
# --------------------------------------------------------------------------- #
class PathManager:
    """Find a writable project root and create the directory hierarchy for a run."""
    ...
    def _resolve_project_root(self) -> Path:
        """Determine a writable directory to store all experiment data."""
    ...
    def build_paths(self, env_id: str, algo: str, run_name: str | None) -> dict[str, Path]:
        """Create the folder tree for a single run and return a dict of useful paths."""
...
# --------------------------------------------------------------------------- #
# 3. EnvFactory – Environment creation, Mario adaptation, pixel packing, spatial filtering
# --------------------------------------------------------------------------- #
class EnvFactory:
    """All environment‑related utilities are encapsulated here."""
    ...
    def _setup_mario_flag(self) -> None:
        """Detect whether the optional Super Mario Bros packages are installed."""
        try:
    ...
    def is_image_env(env_id: str) -> bool:
        """Return True for environments that output raw pixel frames."""
        return env_id.startswith("SuperMarioBros")

    # ---------- Gym → Gymnasium adaptation ----------
    @staticmethod
    def _to_gymnasium_space(space):
        """Convert a classic‑Gym space object to the equivalent Gymnasium space."""
    ...
    def _to_uint8(arr: np.ndarray) -> np.ndarray:
        """Internal uint8 conversion (same logic as Util.to_uint8)."""
        if isinstance(arr, np.ndarray) and arr.dtype == np.uint8:
            return arr
        a = np.array(arr)
        if a.dtype.kind == "f":
    ...
    def make_env(self, env_id: str, seed: int = 0) -> Callable[[], gym.Env]:
        ...
        def _init():
            if self.is_image_env(env_id):
                ...
                # 1. raw env + JoypadSpace (old gym) → bridge to gymnasium
                ...
                # 2. pixel preprocessing pipeline
    ...
    # ---------- Helper for probing the action space ----------
    def probe_action_space(self, env_id: str, seed: int = 0):
        """Create a temporary VecEnv and return its action space (used for algorithm filtering)."""
    ...
    def algo_options_for_space(action_space) -> list[str]:
        """Return a list of algorithm names compatible with *action_space*."""
...
# --------------------------------------------------------------------------- #
class ModelFactory:
    """Package ``algo``, ``policy``, ``model_kwargs`` etc. into Stable‑Baselines3 model objects."""

    @staticmethod
    def policy_for(env_id: str) -> str:
        """Return the SB3 policy name required for *env_id*."""
    ...
    def build_model(
        ...
        """Instantiate the SB3 model (common arguments are handled here)."""
...
class CallbackFactory:
    """Create all callbacks while preserving the original script's behaviour."""
...
# --------------------------------------------------------------------------- #
# 6. Runner – Main process class
# --------------------------------------------------------------------------- #
class Runner:
    """
    Entry point that orchestrates the whole workflow.
    It can run **interactively** (default) or **non‑interactively** when
    ``--env`` / ``--algo`` are supplied on the command line.
    """

    def __init__(
        self,
        env_id: str | None = None,      # <-- optional CLI value
        algo: str | None = None,        # <-- optional CLI value
        interactive: bool = True,       # <-- flag to enable/disable prompts
    ):
        # Core helper objects (unchanged)
        self.util = Util()
        ...
        # Public attribute kept for backward compatibility
        self.project_root = self.path_manager.project_root

        # Store CLI‑provided values and the interactive flag
        self._forced_env_id = env_id
        self._forced_algo = algo
        self.interactive = interactive

    # ------------------------------------------------------------------- #
    # Helper: return a value from CLI / forced value or ask the user
    # ------------------------------------------------------------------- #
    def _get_or_ask(self, kind: str, prompt: str, default: Any) -> Any:
        """
        ``kind`` can be one of: "choice", "int", "yes_no", "str".
        If a forced value (from CLI) exists, it is returned immediately.
        If ``self.interactive`` is False, *default* is returned.
        Otherwise the appropriate ``Util.ask_*`` method is called.
        """
        forced_attr = f"_forced_{kind}"
        forced_val = getattr(self, forced_attr, None)
        ...
        if kind == "choice":
            return self.util.ask_choice(prompt, default)   # ``default`` is a list here
        if kind == "int":
            return self.util.ask_int(prompt, default)
        if kind == "yes_no":
            return self.util.ask_yes_no(prompt, default)
        if kind == "str":
            return self.util.ask_str(prompt, default)
    ...
    def interactive_setup(self):
        """Collect all configuration values – either by prompting or by using defaults."""
        ...
        # -------------------------------------------------
        if self._forced_env_id is not None:
            self.env_id = self._forced_env_id
        else:
        ...
        # -------------------------------------------------
        # 4️⃣ Algorithm selection (CLI may have forced it)
        # -------------------------------------------------
        if self._forced_algo is not None:
            if self._forced_algo not in allowed_algos:
                print(f"[Error] Algorithm '{self._forced_algo}' is not compatible with the env's action space.")
                sys.exit(1)
            self.algo = self._forced_algo
        else:
            self.algo = self.util.ask_choice("Algorithm", allowed_algos)

        # -------------------------------------------------
        # 5️⃣ Load default configuration for the chosen env/algo
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # 6️⃣ Remaining hyper‑parameters
        # -------------------------------------------------
        if self.interactive:
            self.total_timesteps = self.util.ask_int(
                "Total timesteps", self.algo_cfg["train"]["total_timesteps"]
            )
            self.seed = self.util.ask_int("Random seed", 0)
            self.eval_freq = self.util.ask_int(
                "Eval frequency", self.algo_cfg["train"]["eval_freq"]
            )
            ...
            # Non‑interactive mode – use defaults from the config file
            self.total_timesteps = self.algo_cfg["train"]["total_timesteps"]
            self.seed = 0
        ...
        # -------------------------------------------------
        # 7️⃣ Disable VecNormalize for pixel environments (same as original script)
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # 8️⃣ Create the directory hierarchy for this run
        # -------------------------------------------------
        self.paths = self.path_manager.build_paths(self.env_id, self.algo, self.run_name)

    # ------------------------------------------------------------------- #
    # Build vectorised environments (training & evaluation)
    # ------------------------------------------------------------------- #
    def _build_vec_envs(self):
        ...
        # For pixel environments SB3 expects (C, H, W)
    ...
    # ------------------------------------------------------------------- #
    # Build callbacks (evaluation & checkpoint)
    # ------------------------------------------------------------------- #
    ...
    # ------------------------------------------------------------------- #
    # Dump the full run configuration to JSON (useful for reproducibility)
    # ------------------------------------------------------------------- #
    def _dump_config(self):
        run_config = {
            "env_id": self.env_id,
            "algo": self.algo,
            "policy": self.model_factory.policy_for(self.env_id),   # <-- corrected field
            "seed": self.seed,
...
# --------------------------------------------------------------------------- #
#  CLI parsing utilities (only needed for the non‑interactive mode)
# --------------------------------------------------------------------------- #
def parse_cli_args() -> dict:
    """Parse the tiny command‑line interface used by ``nohup`` / scripts."""
    parser = argparse.ArgumentParser(
        description=(
            "Stable‑Baselines3 runner – interactive by default, "
            "but can be driven completely from the command line."
        )
    )
    parser.add_argument(
        "--env", type=str, help="Environment ID to run (e.g. 'CartPole-v1')."
    )
    parser.add_argument(
        "--algo", type=str, help="Algorithm name (e.g. 'PPO', 'DQN')."
    )
    parser.add_argument(
        "--no-interactive",
        action="store_true",
        help="Do not ask any questions – use defaults for everything "
             "except the supplied --env/--algo.",
    )
